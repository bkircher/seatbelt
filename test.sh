...
PI_AUTH_FILE="$HOME_DIR/.pi/agent/auth.json"
...
assert_allowed "can stat /Users" /usr/bin/stat -f "%N" "$USERS_DIR"
...
assert_allowed "can write Pi auth file" /bin/sh -c "printf '%s\n' '{}' > \"\$1\"" sh "$PI_AUTH_FILE"
